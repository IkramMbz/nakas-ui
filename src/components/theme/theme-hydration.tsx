...
export function generateHydrationScript(defaultTheme: Theme): string {
...
export function generateSSRStyles(theme: Theme, mode: ThemeMode): string {
...
export function generateFontPreloadLinks(
  theme: Theme
): Array<{ href: string; as: string; rel: string }> {
  const fonts = new Set([theme.fonts.heading, theme.fonts.body]);
  const links: Array<{ href: string; as: string; rel: string }> = [];
...
export function generateFontStyleLinks(
  theme: Theme
